...
SEG_COLORS  = {"Stable":"#2ecc71","Decaying":"#f39c12","Erratic":"#3498db"}
...
def run_pipeline(path="patient_data.csv"):
    ...
    # Random Forest — best model on this data by CV AUC
    model = RandomForestClassifier(n_estimators=100, max_depth=8, random_state=42)
    model.fit(df[FEATURE_COLS], df['churn_label'])
    ...
    # SHAP — Random Forest returns 3D array (n_samples, n_features, n_classes)
    explainer = shap.TreeExplainer(model)
    shap_vals_raw = explainer.shap_values(df[FEATURE_COLS])
    ...
    df['top_driver'] = [FEATURE_COLS[np.argmax(np.abs(shap_vals[i]))] for i in range(len(df))]
...
if not os.path.exists("patient_data.csv"):
    st.warning("⚠️ No data found. Please run Notebook 1 to generate `patient_data.csv`, then upload it using the sidebar.")
    st.stop()
...
if page == "📊 Dashboard":
    ...
    with c1:
        ...
        fig.add_vline(x=p70, line_dash='dash', line_color='#e74c3c', annotation_text='High')
        fig.add_vline(x=p40, line_dash='dash', line_color='#f39c12', annotation_text='Medium')
...
# ── RISK SCORING ──────────────────────────────────────────────────────────────
    ...
    st.caption("Random Forest classifier — selected automatically by cross-validated AUC.")
    st.markdown("---")
    ...
    with c2:
        ...
        fig2.add_trace(go.Scatterpolar(r=list(avg_n)+[avg_n[0]], theta=FEATURE_COLS+[FEATURE_COLS[0]],
                                        fill='toself', name='Population Avg', line_color='#7C6FEB',
                                        fillcolor='rgba(124,111,235,0.15)'))
        fig2.add_trace(go.Scatterpolar(r=list(pat_n)+[pat_n[0]], theta=FEATURE_COLS+[FEATURE_COLS[0]],
                                        fill='toself', name=pid, line_color='#e74c3c',
                                        fillcolor='rgba(231,76,60,0.15)'))
        fig2.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0,1])), template='plotly_dark')
        st.plotly_chart(fig2, use_container_width=True)
        c_a,c_b = st.columns(2)
        c_a.metric("Risk Score", f"{row['risk_score']:.4f}")
        c_b.metric("Risk Tier",  row['risk_tier'])

# ── SURVIVAL ──────────────────────────────────────────────────────────────────
elif page == "⏱️ Survival Analysis":
    st.title("⏱️ Survival Analysis")
    st.caption("Kaplan-Meier curves show *when* patients disengage. Cox PH shows *what* drives dropout timing.")
    st.markdown("---")
    ...
    for tier in ['High','Medium','Low']:
        ...
        fig.add_trace(go.Scatter(
            ...
            fillcolor=TIER_COLORS[tier].replace('#','rgba(').replace(')',',0.10)'),
            line=dict(color='rgba(255,255,255,0)')
        ))
    fig.update_layout(template='plotly_dark', xaxis_title='Days Since Enrollment',
                      yaxis_title='Probability Still Engaged', yaxis_range=[0,1])
    st.plotly_chart(fig, use_container_width=True)
    ...
    hr_colors = ['#e74c3c' if v>1 else '#2ecc71' for v in summary['exp(coef)']]
    fig2 = go.Figure()
    ...
    for i,(idx,row) in enumerate(summary.iterrows()):
        fig2.add_trace(go.Scatter(
            ...
            y=[idx, idx], mode='lines', line=dict(color='#333', width=2), showlegend=False
        ))
    fig2.add_vline(x=1.0, line_dash='dash', line_color='#aaa', annotation_text='HR=1')
    fig2.update_layout(template='plotly_dark', xaxis_title='Hazard Ratio (>1 = faster dropout | <1 = protective)')
    st.plotly_chart(fig2, use_container_width=True)

    st.caption("Significant features: response_latency_hrs (p<0.005) and message_open_rate (p<0.005). Others not significant in Cox — they predict *whether* someone churns, not *how fast*.")
...
# ── SHAP ──────────────────────────────────────────────────────────────────────
    ...
    fig2 = px.bar(shap_mini, x='SHAP', y='Feature', orientation='h',
                  color='SHAP', color_continuous_scale=['#2ecc71','#f5f5f5','#e74c3c'],
    ...
    c_a.metric("Risk Score", f"{df.loc[pidx,'risk_score']:.4f}")
    c_b.metric("Risk Tier",  df.loc[pidx,'risk_tier'])
    c_c.metric("Top Driver", df.loc[pidx,'top_driver'])

    st.subheader("SHAP Dependence — Inactivity Gap")
    fig3 = px.scatter(x=df['inactivity_gap_days'], y=shap_vals[:,FEATURE_COLS.index('inactivity_gap_days')],
                      color=df['risk_tier'], color_discrete_map=TIER_COLORS,
                      labels={'x':'inactivity_gap_days','y':'SHAP value','color':'Risk Tier'},
                      template='plotly_dark', opacity=0.7)
    fig3.add_hline(y=0, line_dash='dash', line_color='#aaa')
    st.plotly_chart(fig3, use_container_width=True)
...
# ── SEGMENTATION ──────────────────────────────────────────────────────────────
    ...
    fig = px.scatter(df, x='PC1', y='PC2', color='segment',
                     ...
                     template='plotly_dark', opacity=0.7)
    st.plotly_chart(fig, use_container_width=True)
    st.caption("k=3 chosen over k=2 (best silhouette) because two clusters just replicates the churn label. k=3 gives clinically actionable archetypes.")
...
# ── INTERVENTION ──────────────────────────────────────────────────────────────
    ...
    st.caption("Segment + risk tier + decay slope → action. Not just a score threshold.")
    st.markdown("---")
    ...
    with c1:
        ...
        act_colors = {'Human Outreach':'#e74c3c','Automated Nudge':'#f39c12','No Action':'#2ecc71'}
    ...
    with c2:
        ...
        fig2  = px.bar(cross, x='segment', y='value', color='action',
                       color_discrete_map=act_colors, template='plotly_dark',
                       labels={'value':'Count','segment':'Segment','action':'Action'})
        st.plotly_chart(fig2, use_container_width=True)
    ...
    st.caption(f"{len(worklist)} patients require immediate human outreach")
    ...
    st.caption("Change a feature value — see how the risk score changes for any patient.")
    pid  = st.selectbox("Select Patient", df['patient_id'].tolist())
    ...
    X_mod = df[FEATURE_COLS].iloc[pidx].values.copy()
    X_mod[FEATURE_COLS.index(feat)] = new_val
    new_score = model.predict_proba(X_mod.reshape(1,-1))[0][1]
    new_tier  = 'High' if new_score>=p70 else ('Medium' if new_score>=p40 else 'Low')

    c1,c2,c3 = st.columns(3)
    c1.metric("Original Score", f"{df.loc[pidx,'risk_score']:.4f}")
    c2.metric("New Score",      f"{new_score:.4f}", delta=f"{new_score-df.loc[pidx,'risk_score']:+.4f}")
    c3.metric("New Tier",       new_tier)
