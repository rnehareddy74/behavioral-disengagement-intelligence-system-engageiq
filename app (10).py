...
def run_pipeline(path="patient_data.csv"):
    ...
    duration_days  = np.random.exponential(scale=90*(1-scores)+5).clip(1,180).astype(int)
    event_observed = (np.random.rand(len(df)) < (0.3+0.6*scores)).astype(int)
    df['duration_days']  = duration_days.tolist()
    df['event_observed'] = event_observed.tolist()
    ...
    dur  = np.array(df["duration_days"].values, dtype=float)
    evnt = np.array(df["event_observed"].values, dtype=float)
    return df, model, shap_vals, explainer, p40, p70, dur, evnt
