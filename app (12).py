...
# ── SURVIVAL ──────────────────────────────────────────────────────────────────
    ...
    # Build tier index mapping once — avoid repeated boolean mask issues
    tier_indices = {tier: np.where(df['risk_tier'].values == tier)[0] for tier in ['High','Medium','Low']}
    ...
    with c1:
        ...
        for tier in ['High','Medium','Low']:
            ...
            if len(idx) < 5:
                continue
            d = dur_arr[idx].astype(float)
            e = evnt_arr[idx].astype(float)
            try:
                ...
                st.warning(f"KM error for {tier}: {km_err}")
    ...
    with c2:
        ...
        for tier in ['High','Medium','Low']:
            idx = tier_indices[tier]
            if len(idx) < 5:
                med_rows.append({"Tier":tier, "Median Days": 0})
                continue
            d = dur_arr[idx].astype(float)
            e = evnt_arr[idx].astype(float)
            try:
...
# ── SEGMENTATION ──────────────────────────────────────────────────────────────
    ...
    st.dataframe(seg_means, width='stretch')
    st.markdown('</div>', unsafe_allow_html=True)
