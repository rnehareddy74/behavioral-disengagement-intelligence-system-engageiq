...
if page == "Dashboard":
    ...
    with c1:
        ...
        fig = px.histogram(df, x='risk_score', color='risk_tier', color_discrete_map=TIER_COLORS, nbins=40, opacity=0.85)
        fig.add_vline(x=p70, line_dash='dash', line_color='crimson', line_width=1.5, annotation_text='High')
        fig.add_vline(x=p40, line_dash='dash', line_color='coral',   line_width=1.5, annotation_text='Medium')
        fig.update_layout(**PT, height=260, legend=dict(orientation="h",y=1.1))
...
# ── SHAP ──────────────────────────────────────────────────────────────────────
    ...
    with c2:
        ...
        fig3 = px.scatter(x=df['inactivity_gap_days'].values, y=shap_vals[:,fidx],
                           ...
                           labels={'x':'Inactivity Gap (Days)','y':'SHAP Value','color':'Tier'})
        fig3.update_traces(marker=dict(size=5))
