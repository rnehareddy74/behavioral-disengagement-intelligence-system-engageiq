...
# ── INTERVENTION ──────────────────────────────────────────────────────────────
    ...
    with c2:
        ...
        cross = pd.crosstab(df["segment"], df["action"]).reset_index()
        cross = cross.melt(id_vars="segment", var_name="Action", value_name="Count")
        fig2  = px.bar(cross, x="segment", y="Count", color="Action",
                       color_discrete_map=ACT_COLORS, template='plotly_dark',
                       labels={'segment':'Segment','Action':'Action'})
        st.plotly_chart(fig2, use_container_width=True)
