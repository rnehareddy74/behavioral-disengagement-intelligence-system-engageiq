...
def run_pipeline(path="patient_data.csv"):
    ...
    df["duration_days"]  = pd.to_numeric(df["duration_days"],  errors="coerce").fillna(30).astype(int)
...
with st.sidebar:
    ...
    st.markdown("---")
    uploaded = st.file_uploader("Upload patient_data.csv", type="csv", label_visibility="collapsed")
    if uploaded:
...
# ── SURVIVAL ──────────────────────────────────────────────────────────────────
    ...
    with c2:
        ...
        for tier in ['High','Medium','Low']:
            ...
            med_rows.append({"Tier":tier, "Median Days": round(kmf.median_survival_time_,1)})
