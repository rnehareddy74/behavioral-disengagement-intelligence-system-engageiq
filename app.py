...
if not os.path.exists("patient_data.csv"):
    st.warning(" No data found. Please run Notebook 1 to generate `patient_data.csv`, then upload it using the sidebar.")
    st.stop()
...
# ── SURVIVAL ──────────────────────────────────────────────────────────────────
    ...
    for i,(idx,row) in enumerate(summary.iterrows()):
        fig2.add_trace(go.Scatter(
            ...
            y=[idx, idx], mode='lines', line=dict(color='darkgray', width=2), showlegend=False
        ))
